<div class="pricesblock pb30">
    <div class="wrap">
        <h2 class="blocktitle"> Популярные заказы и цены </h2>
        <div class="z7tabs">
            <div class="tabshead th1">
                <ul>
                    <li class="active" onClick="tab(1)">Все</li>
                    <li onClick="tab(2)">Сантехника</li>
                    <li onClick="tab(3)">Электрика</li>
                    <li onClick="tab(4)">Ремонт мебели</li>
                    <li onClick="tab(5)">Мелкий ремонт</li>
                    <li onClick="tab(6)">Жена на час</li>
                </ul>
            </div>
            <script>
                function tab(n) {
                    $('.tabshead li.active').removeClass('active');
                    $('.tabshead li:nth-child(' + n + ')').addClass('active');
                    $('.tabs .tab.active').removeClass('active');
                    $('.tabs .tab:nth-child(' + n + ')').addClass('active');
                }
            </script>
            <div class="tabs">
                <!-- все -->
                <div class="tab active">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200" src="/libs/uploads/2018/12/chs-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка ЖК телевизора</span>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/ch-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/ustanovka-bra/" class="ttl">Установка бра</a>
                            <span class="price">
                           от 400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/44444444444444444444-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена лат</span>
                            <span class="price">
                           от 350 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/jjj-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена механизмов шкафа-купе</span>
                            <span class="price">
                           от 1400 руб. </span>
                        </div>
                        <div class="article">

                            <img width="270" height="200" src="libs/uploads/2018/10/akrilovaya-vanna-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка акриловой ванны</span>
                            <span class="price">
                           от 2800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/Suhaya_chistka_kovra_sodoy_v_domashnih_usloviyah_%d0%a1%d1%83%d1%85%d0%b0%d1%8f-%d1%87%d0%b8%d1%81%d1%82%d0%ba%d0%b0-%d0%ba%d0%be%d0%b2%d1%80%d0%b0-%d1%81%d0%be%d0%b4%d0%be%d0%b9-%d0%b2-%d0%b4%d0%be%d0%bc%d0%b0%d1%88%d0%bd%d0%b8%d1%85-%d1%83%d1%81%d0%bb%d0%be%d0%b2%d0%b8%d1%8f%d1%85-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/chistka-kovrov-i-myagkoj-mebeli-na-domu/" class="ttl">Чистка
                                ковра</a>
                            <span class="price">
                           от 1000 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a3%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d0%b2%d0%b0%d0%bd%d0%bd%d0%be%d0%b9-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/uborka-sanuzlov-vannyh-i-tualetov/" class="ttl">Уборка
                                ванной</a>
                            <span class="price">
                           от 150 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/484446755_w0_h0_uborka_ofisa-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="uslugi-klininga/uborka-ofisov/" class="ttl">Уборка офиса</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/uborka-kvartir/" class="ttl">Уборка квартиры</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a3%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d1%88%d0%b5%d1%80%d1%81%d1%82%d0%b8-%d0%b6%d0%b8%d0%b2%d0%be%d1%82%d0%bd%d0%be%d0%b3%d0%be-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Уборка шерсти</span>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%92%d1%8b%d0%bd%d0%be%d1%81-%d0%bc%d1%83%d1%81%d0%be%d1%80%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="uslugi-klininga/vynos-musora/" class="ttl">Вынос мусора</a>
                            <span class="price">
                           от 250 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d1%87%d0%b8%d1%81%d1%82%d0%ba%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/zhena-na-chas/chistka-zhalyuzi/" class="ttl">Чистка жалюзи</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%9c%d1%8b%d1%82%d1%8c%d0%b5-%d0%bf%d0%be%d1%81%d1%83%d0%b4%d1%8b-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Мытье посуды</span>
                            <span class="price">
                           от 300 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a1%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d0%bc%d0%be%d0%b9%d0%b4%d0%be%d0%b4%d1%8b%d1%80%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Сборка мойдодыра</span>
                            <span class="price">
                           от 700 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/maxresdefault-3-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена петель</span>
                            <span class="price">
                           от 320 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d1%80%d0%b5%d0%bc%d0%be%d0%bd%d1%82_%d1%88%d0%ba%d0%b0%d1%84%d0%b0-%d0%ba%d1%83%d0%bf%d0%b5_2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/remont-mebeli/remont-shkafov-kupe/" class="ttl">Ремонт шкафа-купе</a>
                            <span class="price">
                           от 1400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/ljuhyt-1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async"
                                 srcset="https://masterabyta.ru/libs/uploads/2018/09/ljuhyt-1-270x200.jpg 270w, https://masterabyta.ru/libs/uploads/2018/09/ljuhyt-1.jpg 300w"
                                 sizes="(max-width: 270px) 100vw, 270px" /> <a href="/remont-mebeli/remont-krovati/"
                                                                               class="ttl">Ремонт кровати</a>
                            <span class="price">
                           от 1550 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/62378626_w0_h0_2_sht._img6499-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/remont-mebeli/remont-divanov/" class="ttl">Ремонт дивана</a>
                            <span class="price">
                           от 1500 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/img_8274-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/remont-mebeli/peretyazhka-i-remont-stulev/" class="ttl">Ремонт стула</a>
                            <span class="price">
                           от 650 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/0000-3ab3030l-funnatic-1455707342-1400x1400-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-plotnika/ustanovka-varochnoj-paneli/" class="ttl">Установка варочной
                                панели</a>
                            <span class="price">
                           от 950 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/porozhki-dlja-pola-s-perepadom-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-porozhkov/" class="ttl">Укладка порожков
                                на пол</a>
                            <span class="price">
                           от 250 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/10953_f_6_povesim-televizor-na-stenu.montazhnavesustanovka-televizora-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-televizora-na-stenu/" class="ttl">Монтаж
                                телевизора на стену</a>
                            <span class="price">
                           от 1290 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/rulonnie-shtori-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async"
                                 srcset="https://masterabyta.ru/libs/uploads/2018/09/rulonnie-shtori-270x200.jpg 270w, https://masterabyta.ru/libs/uploads/2018/09/rulonnie-shtori-e1535838271663.jpg 300w"
                                 sizes="(max-width: 270px) 100vw, 270px" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-rulonnyh-shtor/" class="ttl">Монтаж
                                рулонных штор</a>
                            <span class="price">
                           от 450 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/koltsa-nadevayutsya-zaranee-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/povesit-karniz/" class="ttl">Установка карниза для
                                штор</a>
                            <span class="price">
                           от 480 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/tur1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-turnika/" class="ttl">Монтаж турника</a>
                            <span class="price">
                           от 950 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/images_cms-image-000008123-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/povesit-kartinu/" class="ttl">Навес картины на
                                стену</a>
                            <span class="price">
                           от 350 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/118-30-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/montazh-zerkal/" class="ttl">Навес зеркала</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/100_5808-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-plintusa/" class="ttl">Установка
                                плинтуса</a>
                            <span class="price">
                           от 180 руб. (пог. м) </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/yr8s-0dpngg-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка УЗО</span>
                            <span class="price">
                           от 400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/rozetka-kak-podklyuchit-43-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/ustanovka-rozetok/" class="ttl">Установка розетки</a>
                            <span class="price">
                           от 180 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%ad%d0%bb%d0%b5%d0%ba%d1%82%d1%80%d0%be%d1%89%d0%b8%d1%82%d0%be%d0%ba-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/sborka-elektroshhitov/" class="ttl">Монтаж
                                электрощитовой</a>
                            <span class="price">
                           от 2800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%9b%d1%8e%d1%81%d1%82%d1%80%d0%b0-%d0%bf%d1%80%d0%be%d0%b2%d0%be%d0%b42-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Сборка люстры</span>
                            <span class="price">
                           от 750 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/kak-pomenyat-lampochku-v-spote-podvesnogo-potolka-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка светодиодной лампы</span>
                            <span class="price">
                           от 850 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/lipkaya-lenta-iz-plastika-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/germetizatsiya-shvov-v-vannoj/" class="ttl">Герметизация
                                швов в ванной</a>
                            <span class="price">
                           от 600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%a3%d1%81%d1%82%d1%80%d0%b0%d0%bd%d0%b5%d0%bd%d0%b8%d0%b5-%d0%b7%d0%b0%d1%81%d0%be%d1%80%d0%be%d0%b2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/ustranenie-zasorov-kanalizatsii/" class="ttl">Устранение
                                засоров</a>
                            <span class="price">
                           от 790 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/Remont-un-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/remont-unitaza/" class="ttl">Ремонт унитаза</a>
                            <span class="price">
                           От 800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/211131583_w0_h0_8-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/zamena-smesitelya/" class="ttl">Замена смесителя</a>
                            <span class="price">
                           от 650 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%a0%d0%b0%d0%ba%d0%be%d0%b2%d0%b8%d0%bd%d0%b0-1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/ustanovka-rakoviny-v-vannoj/" class="ttl">Установка
                                раковины</a>
                            <span class="price">
                           от 1200 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%9f%d0%be%d0%b4%d0%ba%d0%bb%d1%8e%d1%87%d0%b5%d0%bd%d0%b8%d0%b5-%d0%b1%d0%b8%d0%b4%d0%b5-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Подключение биде</span>
                            <span class="price">
                           от 1050 руб. </span>
                        </div>
                    </div>
                </div>
                <!-- сантехника -->
                <div class="tab">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/10/akrilovaya-vanna-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка акриловой ванны</span>
                            <span class="price">
                           от 2800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a1%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d0%bc%d0%be%d0%b9%d0%b4%d0%be%d0%b4%d1%8b%d1%80%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Сборка мойдодыра</span>
                            <span class="price">
                           от 700 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/lipkaya-lenta-iz-plastika-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/germetizatsiya-shvov-v-vannoj/" class="ttl">Герметизация
                                швов в ванной</a>
                            <span class="price">
                           от 600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%a3%d1%81%d1%82%d1%80%d0%b0%d0%bd%d0%b5%d0%bd%d0%b8%d0%b5-%d0%b7%d0%b0%d1%81%d0%be%d1%80%d0%be%d0%b2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/ustranenie-zasorov-kanalizatsii/" class="ttl">Устранение
                                засоров</a>
                            <span class="price">
                           от 790 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/Remont-un-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/remont-unitaza/" class="ttl">Ремонт унитаза</a>
                            <span class="price">
                           От 800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/08/211131583_w0_h0_8-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/zamena-smesitelya/" class="ttl">Замена смесителя</a>
                            <span class="price">
                           от 650 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%a0%d0%b0%d0%ba%d0%be%d0%b2%d0%b8%d0%bd%d0%b0-1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-santehnika/ustanovka-rakoviny-v-vannoj/" class="ttl">Установка
                                раковины</a>
                            <span class="price">
                           от 1200 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/08/%d0%9f%d0%be%d0%b4%d0%ba%d0%bb%d1%8e%d1%87%d0%b5%d0%bd%d0%b8%d0%b5-%d0%b1%d0%b8%d0%b4%d0%b5-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Подключение биде</span>
                            <span class="price">
                           от 1050 руб. </span>
                        </div>
                    </div>
                </div>
                <!-- электрика -->
                <div class="tab">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/chs-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка ЖК телевизора</span>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/ch-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/ustanovka-bra/" class="ttl">Установка бра</a>
                            <span class="price">
                           от 400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/0000-3ab3030l-funnatic-1455707342-1400x1400-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-plotnika/ustanovka-varochnoj-paneli/" class="ttl">Установка варочной
                                панели</a>
                            <span class="price">
                           от 950 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/yr8s-0dpngg-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка УЗО</span>
                            <span class="price">
                           от 400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/rozetka-kak-podklyuchit-43-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/ustanovka-rozetok/" class="ttl">Установка розетки</a>
                            <span class="price">
                           от 180 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%ad%d0%bb%d0%b5%d0%ba%d1%82%d1%80%d0%be%d1%89%d0%b8%d1%82%d0%be%d0%ba-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-elektrika/sborka-elektroshhitov/" class="ttl">Монтаж
                                электрощитовой</a>
                            <span class="price">
                           от 2800 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%9b%d1%8e%d1%81%d1%82%d1%80%d0%b0-%d0%bf%d1%80%d0%be%d0%b2%d0%be%d0%b42-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Сборка люстры</span>
                            <span class="price">
                           от 750 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/kak-pomenyat-lampochku-v-spote-podvesnogo-potolka-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Установка светодиодной лампы</span>
                            <span class="price">
                           от 850 руб. </span>
                        </div>
                    </div>
                </div>
                <!-- ремонт мебели -->
                <div class="tab">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/44444444444444444444-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена лат</span>
                            <span class="price">
                           от 350 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/12/jjj-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена механизмов шкафа-купе</span>
                            <span class="price">
                           от 1400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/maxresdefault-3-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Замена петель</span>
                            <span class="price">
                           от 320 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d1%80%d0%b5%d0%bc%d0%be%d0%bd%d1%82_%d1%88%d0%ba%d0%b0%d1%84%d0%b0-%d0%ba%d1%83%d0%bf%d0%b5_2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/remont-mebeli/remont-shkafov-kupe/" class="ttl">Ремонт шкафа-купе</a>
                            <span class="price">
                           от 1400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/ljuhyt-1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async"
                                 srcset="https://masterabyta.ru/libs/uploads/2018/09/ljuhyt-1-270x200.jpg 270w, https://masterabyta.ru/libs/uploads/2018/09/ljuhyt-1.jpg 300w"
                                 sizes="(max-width: 270px) 100vw, 270px" /> <a href="/remont-mebeli/remont-krovati/"
                                                                               class="ttl">Ремонт кровати</a>
                            <span class="price">
                           от 1550 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/62378626_w0_h0_2_sht._img6499-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="remont-mebeli/remont-divanov/" class="ttl">Ремонт дивана</a>
                            <span class="price">
                           от 1500 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a0%d0%b5%d0%bc%d0%be%d0%bd%d1%82-%d0%ba%d1%80%d0%b5%d1%81%d0%bb%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Ремонт кресла</span>
                            <span class="price">
                           от 1400 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/img_8274-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/remont-mebeli/peretyazhka-i-remont-stulev/" class="ttl">Ремонт стула</a>
                            <span class="price">
                           от 650 руб. </span>
                        </div>
                    </div>
                </div>
                <!-- мелкий ремонт -->
                <div class="tab">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/porozhki-dlja-pola-s-perepadom-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-porozhkov/" class="ttl">Укладка порожков
                                на пол</a>
                            <span class="price">
                           от 250 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/10953_f_6_povesim-televizor-na-stenu.montazhnavesustanovka-televizora-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-televizora-na-stenu/" class="ttl">Монтаж
                                телевизора на стену</a>
                            <span class="price">
                           от 1290 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/rulonnie-shtori-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async"
                                 srcset="https://masterabyta.ru/libs/uploads/2018/09/rulonnie-shtori-270x200.jpg 270w, https://masterabyta.ru/libs/uploads/2018/09/rulonnie-shtori-e1535838271663.jpg 300w"
                                 sizes="(max-width: 270px) 100vw, 270px" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-rulonnyh-shtor/" class="ttl">Монтаж
                                рулонных штор</a>
                            <span class="price">
                           от 450 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/koltsa-nadevayutsya-zaranee-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/povesit-karniz/" class="ttl">Установка карниза для
                                штор</a>
                            <span class="price">
                           от 480 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/tur1-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-turnika/" class="ttl">Монтаж турника</a>
                            <span class="price">
                           от 950 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/images_cms-image-000008123-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/povesit-kartinu/" class="ttl">Навес картины на
                                стену</a>
                            <span class="price">
                           от 350 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/118-30-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/montazh-zerkal/" class="ttl">Навес зеркала</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/100_5808-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/melkij-remont-v-kvartire/ustanovka-plintusa/" class="ttl">Установка
                                плинтуса</a>
                            <span class="price">
                           от 180 руб. (пог. м) </span>
                        </div>
                    </div>
                </div>
                <!-- жена на час -->
                <div class="tab">
                    <div class="articles">
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/Suhaya_chistka_kovra_sodoy_v_domashnih_usloviyah_%d0%a1%d1%83%d1%85%d0%b0%d1%8f-%d1%87%d0%b8%d1%81%d1%82%d0%ba%d0%b0-%d0%ba%d0%be%d0%b2%d1%80%d0%b0-%d1%81%d0%be%d0%b4%d0%be%d0%b9-%d0%b2-%d0%b4%d0%be%d0%bc%d0%b0%d1%88%d0%bd%d0%b8%d1%85-%d1%83%d1%81%d0%bb%d0%be%d0%b2%d0%b8%d1%8f%d1%85-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/chistka-kovrov-i-myagkoj-mebeli-na-domu/" class="ttl">Чистка
                                ковра</a>
                            <span class="price">
                           от 1000 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a3%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d0%b2%d0%b0%d0%bd%d0%bd%d0%be%d0%b9-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/uborka-sanuzlov-vannyh-i-tualetov/" class="ttl">Уборка
                                ванной</a>
                            <span class="price">
                           от 150 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/484446755_w0_h0_uborka_ofisa-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/uborka-ofisov/" class="ttl">Уборка офиса</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200" src="libs/uploads/2018/09/2-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/uborka-kvartir/" class="ttl">Уборка квартиры</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%a3%d0%b1%d0%be%d1%80%d0%ba%d0%b0-%d1%88%d0%b5%d1%80%d1%81%d1%82%d0%b8-%d0%b6%d0%b8%d0%b2%d0%be%d1%82%d0%bd%d0%be%d0%b3%d0%be-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Уборка шерсти</span>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%92%d1%8b%d0%bd%d0%be%d1%81-%d0%bc%d1%83%d1%81%d0%be%d1%80%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/uslugi-klininga/vynos-musora/" class="ttl">Вынос мусора</a>
                            <span class="price">
                           от 250 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d1%87%d0%b8%d1%81%d1%82%d0%ba%d0%b0-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <a
                                href="/zhena-na-chas/chistka-zhalyuzi/" class="ttl">Чистка жалюзи</a>
                            <span class="price">
                           от 1600 руб. </span>
                        </div>
                        <div class="article">
                            <img width="270" height="200"
                                 src="libs/uploads/2018/09/%d0%9c%d1%8b%d1%82%d1%8c%d0%b5-%d0%bf%d0%be%d1%81%d1%83%d0%b4%d1%8b-270x200.jpg"
                                 class="attachment-svc size-svc wp-post-image" alt="" decoding="async" /> <span
                                class="ttl">Мытье посуды</span>
                            <span class="price">
                           от 300 руб. </span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>